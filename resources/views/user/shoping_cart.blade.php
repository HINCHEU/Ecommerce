...
0
    ...
    @section('new_js')
        <script>
            ...
            let minPurchaseAmount = 0; // Store minimum purchase amount for coupon
                    ...
                    success: function (data) {
                        ...
                        if (data.cart) {
                            data.cart.forEach(function (item) {
                                const itemSubtotal = ((item.base_price + item.color_additional_price + item.size_additional_price) * item.quanity) * (1 - item.discount / 100);
                        ...
                        $('#apply_coupon').click(function () {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.success) {
                                        // Assuming the backend returns the discount value (percentage or fixed) as a number (e.g., 0.5 or 15)
                                        let discount_total = 0;

                                        // Determine if the discount is percentage or fixed
                                        ...
                                        // Store the min_purchase value from the backend
                                        ...
                                        appliedDiscount = response.discount; // Store the applied discount
                                        updateTotals(cartTotal);  // Update the totals after applying the coupon
                                    } else {
                                        swal({
                                            icon: 'error',
                                            title: 'Oops!',
            ...
            function updateTotals(subtotal) {
                ...
                if (subtotal < minPurchaseAmount) {
                    ...
                    appliedDiscount = 0;
                    discount_total = 0;  // Reset discount to 0
                    alert("Your cart total is below the required minimum for the coupon.");
